...
template <typename T>
RVector<T> RandRVector(const _size_t n, const T min, const T max);
...
inline Real log_base(T a, U base) {
  static_assert(::is_a_number<T> && ::is_a_number<U>,
                "Type passed must be a arithmetic type");
...
struct RealOrInt {
  static_assert(::is_a_number<A> && ::is_a_number<B>,
                "Type passed must be a arithmetic type");
  using type =
      std::conditional_t<std::is_same_v<A, Real> || std::is_same_v<B, Real>,
                         Real, int>;
};
...
class RVector {
 ...
 private:
  ...
  template <typename U>
  friend RVector<U> RandRVector(const _size_t n, const U min, const U max);
 ...
 public:
  ...
  inline T* data_ptr() { return v.get(); }
...
RVector<T> RandRVector(const _size_t n, const T min, const T max) {
  ...
  std::mt19937 gen(
      rd());  // Mersenne Twister random number engine seeded with rd
  if constexpr (std::is_integral<T>::value) {
    std::uniform_int_distribution<> distrib(
        min, max);  // Uniform distribution between min and max inclusive
    for (_size_t i = 0; i < n; i++) {
      vec.v[i] = distrib(gen);
    }
  } else {
...
template <typename T>
RVector<Real> LinSpace(const T min, const T max, const _size_t n) {
  RVector<Real> vec(n);
...
template <typename T>
RVector<Real> GeomSpace(const T min, const T max, const _size_t n,
                        const T base) {
